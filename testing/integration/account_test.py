...
from data.account_test_data import CreateAccountIN, CreateAccountOUT, GetAccountOUT, GetAccountsOUT, DeactivateAccountOUT
...
def test_get_account():
    ...
    assert removeTimestamp(invalidIDResponse.json()) == GetAccountOUT.invalidIDResponse, 'Error cuando el tipo de cuenta es invalido al obtener la cuenta'
    assert removeTimestamp(notFoundResponse.json()) == GetAccountOUT.notFoundResponse, 'Error cuando no hay una cuenta con es ID'

    post(f"{BASE_URL}/user", json=CreateUserIN.validData1)
    post(f"{BASE_URL}/account", json=CreateAccountIN.validData)
    validResponse = get(f"{BASE_URL}/account/1")

    assert removeTimestamp(validResponse.json()) == GetAccountOUT.validResponse, 'Error cuando se obtiene un cuenta valida'
...
def test_get_accounts():
    ...
    assert removeTimestamp(invalidIDResponse.json()) == GetAccountsOUT.invalidIDResponse, 'Error cuando el ID es invalido al obtener las cuentas'
    assert removeTimestamp(notFoundResponse.json()) == GetAccountsOUT.notFoundResponse, 'Error cuando no hay cuentas asignadas a este ID'
    ...
    assert removeTimestamp(validResponse.json()) == GetAccountsOUT.validResponse, 'Error cuando se obtienen las cuentas de un ID valido'
...
def test_deactivate_account():
    ...
    assert removeTimestamp(invalidIDResponse.json()) == DeactivateAccountOUT.invalidID, 'Error cuando el ID es invalido al desactivar una cuenta'
    assert removeTimestamp(notFoundResponse.json()) == DeactivateAccountOUT.notFound, 'Error al no encontrar la cuenta al desactivar una cuenta'

    post(f"{BASE_URL}/user", json=CreateUserIN.validData1)
    post(f"{BASE_URL}/account", json=CreateAccountIN.validData)

    validResponse = delete(f"{BASE_URL}/account/1")
    assert removeTimestamp(validResponse.json()) == DeactivateAccountOUT.validResponse, 'Error al desactivar una cuenta valida'

    alreadyDeleted = delete(f"{BASE_URL}/account/1")
    assert removeTimestamp(alreadyDeleted.json()) == DeactivateAccountOUT.notFound, 'Error al desactivar una cuenta que ya fue desactivada'