...
def test_valid_input():
    ...
    assert account.user_id == IAccountCases.validData["user_id"], \
        'Error al obtener ID de una validacion correcta'
    assert account.account_type == IAccountCases.validData["account_type"], \
        'Error al obtener tipo de cuenta de una validacion correcta'
...
def test_invalid_user_id():
    ...
    assert excinfo.value.errors()[0]['msg'] == \
        "Value error, La cedula no es valida", \
        'Error al validar ID invalido'
...
def test_invalid_account_type():
    ...
    assert excinfo.value.errors()[0]['msg'] == \
        "Value error, El tipo de cuenta no es valida", \
