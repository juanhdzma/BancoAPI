...
class TransactionDAO(TransactionRepository):
    ...
    def hacerTransferencia(self, transaction):
        try:
            ...
            session.query(Account).filter_by(id=destination).update(
    ...
    def historialTransferencias(self, idAccount):
        try:
            session = self.database.createConnection()
            response = session.query(Transaction)\
                .filter(or_(Transaction.source_account == idAccount,
                            Transaction.destination_account == idAccount)).all()
            return response
        except BaseException:
            return False
        finally:
